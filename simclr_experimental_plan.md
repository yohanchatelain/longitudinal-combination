...
1. **Possible indirect NKI leakage via CoRR — investigated, bounded, not fully resolvable from public data.** SimCLR's Table 1 includes CoRR (Consortium for Reliability and Reproducibility, Zuo et al. 2014) at 1,416 patients. CoRR aggregates two NKI-Rockland-derived sites directly:
   - `nki_2` (pediatric test-retest, 77 subjects, `A00XXXXXX`-format IDs — the same ID convention this repo's own NKI cohort file uses). **Not a risk for this plan's scope**: §3 already restricts this comparator arm to `Longitudinal_Adult` subjects, so the pediatric CoRR site cannot overlap with it.
   - `nki_1` (adult test-retest, **24 subjects**, `NKI_TRT_XXXXXXX`-format IDs, described by CoRR's own documentation as "primarily composed of individuals from the initial NKI-RS"). This is the live risk: up to 24 of this plan's 200 `Longitudinal_Adult` subjects could in principle be among SimCLR's pretraining data.

   CoRR publishes no crosswalk between `NKI_TRT_*` IDs and NKI-Rockland's `A00XXXXXX` IDs, and the associated phenotypic/demographic file (`NKI_TRT_Phenotypic_Data`) is gated behind a NITRC login — not retrievable without a data-use account, which was not available to resolve this during plan drafting. Exact subject-level overlap therefore **cannot be determined from public sources**.

   **Decision: accept as a documented, bounded residual risk rather than a blocker.** Worst case is ≤24 of 200 `Longitudinal_Adult` subjects (≤12%) — materially different from the PPMI situation in §1, where overlap was total and confirmed by construction. A clean resolution requires a NITRC account with access to `NKI_TRT_Phenotypic_Data`, cross-referenced against this repo's `cohort_longitudinal.csv` by age/sex/handedness as a best-effort screen (exact match is not guaranteed even then, since fields like age can coincide by chance in a 200-subject sample). **Before reporting any confirmatory result from this plan, either obtain that crosswalk and exclude matches, or explicitly caveat that up to 12% of the `Longitudinal_Adult` sample may carry unverified SimCLR-pretraining exposure.**
